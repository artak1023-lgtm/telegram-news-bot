...
OWNER_ID = int(os.environ.get('OWNER_ID', '0'))  # Քո Telegram User ID-ն
...
# Conversation states
WAITING_SOURCE_NAME, WAITING_SOURCE_URL, WAITING_KEYWORD_ADD, WAITING_ADMIN_ID = range(4)
...
admin_list = set()  # Admin user IDs
admin_notifications = {}  # {user_id: True/False} - արդյոք admin-ը ուզում է ծանուցումներ

def load_admins():
    """Load admin list from file"""
    global admin_list, admin_notifications
    try:
...
def save_admins():
    ...
    try:
        ...
        logger.info(f"Saved {len(admin_list)} admins")
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"Բարի գալուստ, @{username}!{admin_status}\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Admin-only actions
    admin_only_actions = ['admins', 'add_admin', 'remove_admin', 'list_admins', 'toggle_notifications']
    ...
    if query.data == 'admins':
        ...
        # Show list of admins to remove (except owner)
        keyboard = []
        ...
        for admin_id in admin_list:
            ...
            admin_info.append(f"{role} {notif} - <code>{admin_id}</code>")
        
        msg = "👥 <b>Admin-ների ցուցակ</b>\n\n" + "\n".join(admin_info)
        ...
        if not is_admin(user_id):
            await query.answer("❌ Միայն admin-ները կարող են դիտել", show_alert=True)
            return
        ...
        if not is_admin(user_id):
            await query.answer("❌ Միայն admin-ները կարող են ավելացնել", show_alert=True)
            return
        ...
        if not is_admin(user_id):
            await query.answer("❌ Միայն admin-ները կարող են հեռացնել", show_alert=True)
            return
        ...
        if not is_admin(user_id):
            await query.answer("❌ Միայն admin-ները կարող են փոխել", show_alert=True)
            return
...
async def notify_admins(context: ContextTypes.DEFAULT_TYPE, message: str):
    """Send notification to all admins who have notifications enabled"""
    for admin_id in admin_list:
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle text messages for adding sources, keywords, and admins"""
    ...
    if waiting_for == 'admin_id':
        ...
        try:
            ...
            admin_notifications[new_admin_id] = True  # Enable notifications by default
            save_admins()
            ...
            try:
                await context.bot.send_message(
                    ...
                    text="🎉 <b>Դու admin ես դարձել!</b>\n\n"
                         ...
                         "• Ստանալ ծանուցումներ փոփոխությունների մասին\n\n"
...
async def my_id_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Անուն՝ {first_name}\n"
        f"Username՝ @{username}\n"
        f"User ID՝ <code>{user_id}</code>{admin_status}",
        parse_mode='HTML'
    )
...
async def check_news_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for article in new_articles_sorted:
        try:
            ...
            # Send to admins who have notifications enabled
            for admin_id in admin_list:
                if admin_notifications.get(admin_id, True):
                    try:
                        await context.bot.send_message(
